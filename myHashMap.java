 ...
 * contains in the map. int size() - returns the number of <k,v> pairs in
 * hashmap boolean isEmpty() - returns true if this map contains no key-value
 * mappings.
 *
 *
 * Methods *NOT* implemented to fully emulate the behavior of Java's HashMap
 * Class - clone() - compute() - computeIfAbsent() - computeIfPresent() -
 * foreach() - merge(() - putAll() - replaceAll() - values()
 *
 ***************************************
 */
 ...
 * Interface. The <key, values> pairs are stored in a Map, where the key
 * represents a hash bucket slot number and the value represents a node which
 * will form as linked-list for hash collisions on that bucket's slot.
 *
 ...
 * array is generated using a hash function that returns a number from 0 to n-1,
 * where n is the number of buckets (array size).
 *
 ...
 * same level. We are illustrating 'separate chaining' using a singly
 * linked-list.
 *
 * Last, the hashmap (array) is small, in practice, it will be much larger. But
 * this will also illustrate the load factor being reached much faster and
 * seeing the hashmap growth code be exercised.
 */
class myHashMap<K, V> {
     ...
     *
     * Performs two parts. 1) First invokes a very simple hash code generator
     * which generates a 32-bit integer. The mask (bit operation) masks off the
     * sign bit )turns the 32-bit integer into a 31-bit non-negative integer).
     ...
     * (numBuckets-1) which will be an index into our hash bucket slot (aka, key
     * for Map);
     *
     ...
     * map contains no mapping for the key. This method will probe to the
     * correct bucket, then loop through the bucket's chained nodes
     * (linked-list) until the key is found. If not found, the key is not in the
     * hash map and the method will return null.
     *
     ...
     * the specified value. The method will probe into the bucket lists. If the
     * bucket has a chained list, then it will be traversed to identify the key
     * to remove. If no chained list and/or no node is found the chained list
     * representing the key, then it is not in the hash map.
     *
     * If the key is found, it is removed from the chained list and the hashmap
     * size is adjusted.
     *
    ...
    public V remove(K key) {
        int index = getBucketIndex(key);  // Determine the correct bucket.
        HashNode<K, V> head = bucket.get(index);
        HashNode<K, V> prev = null;

        // Traverse the linked list at the bucket to find the node to remove.
        while (head != null) {
            if (head.key.equals(key)) {
                // If this node is at the head of the list.
                if (prev == null) {
                ...
                size--; // Adjust the hashmap's size.
     ...
     * method will check if the key is already in the hash map, if so, it
     * updates the value and returns the old value. If the key is not found,
     * then it will insert the
     * <k,v> pair. Last if inserting the <k,v>, the load factor is checked. If
     * it is greater than the DEFAULT_LOAD_FACTOR %, the method will double the
     * bucket map and rehash the whole hash map.
     *
     * @param key - Key to the <k,v> pair operate on
     * @param value - if key found, value is updated to this param, else routine
     * inserts <k,v>
     *
     * @return value - if key exists, returns old value before replacing with
     * provided value, else null.
     */
     ...
     * @parem: value - The value to place in as a <k, v> pair if key does not
     * exist
     *
     ...
     * to some value (aka, the key already exist with some value).
     *
     * @param key - Key for the <k, v> pair to replace its value
     * @param val - The new value to replace the old one if found.
     *
     * @return V - returns the old value for the <k,v> pair, else null if not
     * found.
     */
     ...
     *
     * @param val: Value to search for in hashmap to determine if it is
     * contained there.
     *
